...
export const PaymentStatus = ({ status }) => {
  ...
  return (
    <div style={{ color: STATUS_COLOR[status] }}>{statusList[status]}</div>
  );
};
...
function ListReceipt() {
  ...
  function handleUpdateState(record) {
    ReceiptionService.updateReceiptById({
      id: record.customerReceiptId,
      data: {
  ...
  let columns = [
    ...
    },
    {
      ...
      render: (value, values) => {
        const color = values?.schedule?.licensePlateColor
          ? values?.schedule?.licensePlateColor - 1
          : 0;
        return (
    ...
    },
    {
      ...
      render: (_, rowData) => {
        const isUnConfimred =
          rowData.customerReceiptStatus === PAYMENT_STATE.NEW;
        ...
        return (
          <>
            ...
            {isUnConfimred && (
              ...
              >
                <Button type="link">{translation("receipt.cancel")}</Button>
              </Popconfirm>
            )}
            {/* <Popover
              placement="topRight"
    ...
    await setTimeout(() => {
      ...
      onExportExcel({
        ...
        informationColumn: [
          ...
          [
            `Mã: Trung Tâm đăng kiểm xe cơ giới ${user?.stationCode || "123"}`,
            "",
            "",
            `Danh sách hóa đơn ngày ${moment().format("DD/MM/YYYY")}`,
          ],
          [""],
        ],
        timeWait: 0,
            ...
            {loading ? (
              ...
              <>
                ...
                <BasicTablePaging
                  ...
                  skip={dataFilter.skip || 0}
                  count={dataReceipt?.data?.length < dataFilter.limit}
                ></BasicTablePaging>
              </>
            )}
          </div>
