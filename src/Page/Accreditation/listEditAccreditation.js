...
function ListEditAccreditation() {
  ...
  const OptionsColor = [
    {
      value: "white",
      lable: (
        <div className="accreditation__parent">
  ...
  function handleUpdateNewData(updateData) {
    setDataAccreditation((prevData) => {
      const index = prevData.data.findIndex(
        (item) => item.customerRecordId === updateData.customerRecordId
      );
      if (index !== -1) {
  ...
  function handleAddNewData(addData) {
    setDataAccreditation((prevData) => {
      const index = prevData.data.findIndex(
        (item) => item.customerRecordId === addData.customerRecordId
      );
      if (index === -1) {
        const newData = [];
  ...
  function handleDeleteData(deleteData) {
    setDataAccreditation((prevData) => {
      const newData = prevData.data.filter(
        (item) => item.customerRecordId !== deleteData.customerRecordId
      );
      return {
        total: prevData.total - 1,
        data: newData,
  ...
  const columns = [
    {
      title: (
        <Typography.Paragraph className="accreditation__title">
          {translation("listDocumentary.index")}
    ...
    },
    {
      title: (
        <Typography.Paragraph className="accreditation__title">
          {translation("accreditation.licensePlates")}
        </Typography.Paragraph>
      ),
  ...
  function handleSave(row, key, isReload) {
    ...
    if (key === "customerRecordCheckDuration") {
      customerRecordCheckExpiredDate = moment(
        row.customerRecordCheckDate,
        "DD/MM/YYYY"
      )
        .add(Number(row.customerRecordCheckDuration), "months")
        .subtract(1, "day")
        .format("DD/MM/YYYY");
      handleUpdateById(
        {
  ...
  function renderTextOrPlaceholder(
    record,
    dataIndex,
  ...
  const columnsV2 = [
    ...
    },
    {
      ...
      render: (_, record) =>
        renderTextOrPlaceholder(
          record,
          "customerRecordPhone",
          "accreditation.phoneNumber"
        ),
    },
    {
      title: (
        <Typography.Paragraph className="accreditation__title">
          {translation("accreditation.inspectionDate")}
        </Typography.Paragraph>
      ),
    ...
    },
    {
      title: (
        <Typography.Paragraph className="accreditation__title">
          {translation("accreditation.accreditationStatus")}
        </Typography.Paragraph>
      ),
      ...
      render: (value, record) => {
        ...
        return (
          <div className="d-inline-flex">
            <CompletedShowIcon
              status={value}
              onClick={(status) => handleUpdateStatus(record, status)}
              confirm={true}
            />
          </div>
        );
    ...
    },
    {
      ...
      componentInput: (inputRef, save, form, setEditing) => {
        return renderDatePicker(
          ...
          "accreditation.inspectionExpireDate",
          inputRef,
          save,
          setEditing
        );
    ...
    },
    {
      ...
      render: (_, record) => {
        const value =
          record && record.customerRecordCheckDuration
            ? `${record.customerRecordCheckDuration} ${translation(
                "accreditation.month"
              )}`
            : translation("accreditation.inspectionCycle");
  ...
  const SCHEDULE_ERROR = useMemo(() => {
    return {
      INVALID_STATION: translation(
        "accreditation.scheduleError.invalidStation"
      ),
      INVALID_BOOKING_CONFIG: translation(
        "accreditation.scheduleError.invalidBookingConfig"
      ),
      BOOKING_MAX_LIMITED: translation(
        "accreditation.scheduleError.bookingMaxLimited"
      ),
      UNCONFIRMED_BOOKING_EXISTED: translation(
        "accreditation.scheduleError.unconfirmedBookingExisted"
      ),
    };
  }, [translation]);
  ...
  return (
    <>
      <Tabs
        activeKey={ListEditAccreditationKey}
        onChange={(key) => {
      ...
      >
        <TabPane
          tab={translation("accreditation.title")}
          key={ListAccreditationKey}
        />
        <TabPane
          tab={translation("accreditation.list")}
          key={ListEditAccreditationKey}
        >
          <Fragment>
              ...
              <div>
                  ...
                  <div className="list_accreditation_header mb-3">
                    ...
                    <Space size={24} wrap className="justify-content-md-center">
                      <div>
                        <Form
                          initialValues={{
                            customerRecordCheckDate: convertStrToDate(
                              dataFilter.filter?.customerRecordCheckDate,
                              "DD/MM/YYYY"
                            ),
                            customerRecordCheckExpiredDate: convertStrToDate(
                              dataFilter.filter?.customerRecordCheckExpiredDate,
                              "DD/MM/YYYY"
                            ),
                          }}
                                ...
                                {(values) => {
                                  return (
                                    <DatePicker
                                      ...
                                      initialValues={values.getFieldValue(
                                        "customerRecordCheckDate"
                                      )}
                ...
                {loading ? (
                  ...
                  <Table
                    rowClassName={(record) =>
                      `${
                        record && record.customerRecordModifyDate
                          ? "edited-row editable-row"
                          : "editable-row"
                ...
                <BasicTablePaging
                  ...
                  count={dataAccreditation?.data?.length < dataFilter.limit}
                ></BasicTablePaging>
                <Modal
                  visible={isCreate}
                  ...
                  <Form onFinish={onCreateNewCustomer} form={formModal}>
                    <div className="row">
                      <div className="col-12 col-md-12">
                        <Form.Item name="customerRecordPlatenumber">
                          <Input
                            autoFocus
        ...
        {!isHiddenTabs(InspectionProcessKey) && (
          <TabPane
            tab={translation("header.inspectionProcess")}
            key={InspectionProcessKey}
          />
        )}
        {!isHiddenTabs(AccreditationNotificationKey) && (
          <TabPane
            tab={translation("accreditation.accreditationNotification")}
            key={AccreditationNotificationKey}
          />
        )}
        {!isHiddenTabs(AccreditationStatisticalKey) && (
          <TabPane
            tab={translation("listCustomers.statistical")}
            key={AccreditationStatisticalKey}
          />
        )}
        {!isHiddenTabs(ListReportStaistic) && (
          <TabPane
            tab={translation("accreditation.statistic")}
            key={ListReportStaistic}
          />
        )}
      </Tabs>
      {!!crimePlateNumber && (
