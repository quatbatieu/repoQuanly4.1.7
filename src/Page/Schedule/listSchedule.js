...
import {
  SCHEDULE_STATUS_STATES_EXPORT,
  SCHEDULE_STATUS_STATES_IMPORT,
  LICENSE_PLATE_COLOR_STATES_EXPORT,
  LICENSE_PLATE_COLOR_STATES_IMPORT,
...
const ListSchedule = () => {
  ...
  const columns = [
      ...
      render: (value, scheduleItem) => {
        return (
          <span
            onClick={() =>
              history.push(
                `${routes.customerSchedule.path}/${scheduleItem.customerScheduleId}`
              )
            }
          >
            {value}
    ...
    },
    {
      ...
      render: (value, record) => {
        return (
          <div>
            <div>
              {VEHICLE_SUB_TYPES?.[record.vehicleSubType] ||
                VEHICLE_SUB_TYPES?.[value] ||
                "---"}
            </div>
            <div>
              {VEHICLE_SUB_CATEGORY?.[record.vehicleSubCategory] || "---"}
            </div>
          </div>
        );
      },
        ...
        return (
          <div>
            ...
            <span
              ...
              onClick={() =>
                history.push(
                  `${routes.customerSchedule.path}/${record.customerScheduleId}`
                )
              }
            >
    ...
    },
    {
      ...
      render: (value, scheduleItem) => {
        if (value?.length === 0) {
          return (
            Object.values(metaData?.SCHEDULE_TYPE).find(
              (obj) => obj?.scheduleType === scheduleItem?.scheduleType
            )?.scheduleTypeName || "---"
          );
        }
      ...
      render: (value) => {
        return (
          <Typography.Paragraph
            style={{ color: SCHEDULE_STATUS[value]?.color || "" }}
            className="mb-0"
          >
      ...
      render: (record, scheduleItem) => {
        ...
        return (
          <div className="d-inline-flex align-items-center">
            ...
            {(isConfirm || isUnConfimred) &&
              USER_ROLES.ADMIN === user.appUserRoleId && (
                <>
                  <Button
                    type="primary"
                  ...
                  <ModalDeleteCalendar
                    onOk={(message) => {
                      onDeleteSchedule(
                        record && record.customerScheduleId,
                        message
                      );
                    }}
                  />
            ...
            {!!setting.enablePaymentGateway &&
              record?.paymentStatus === PAYMENT_STATE.SUCCESS && (
                <Button
                  type="primary"
                  className="d-inline-flex align-items-center mx-1"
                  onClick={() => {
    ...
    AddBookingService.AddBooking({
      ...
      if (!result.issSuccess) {
        if (Object.keys(SCHEDULE_ERROR).includes(result.error)) {
          ...
          handleStatus(
            translation("addBookingSuccess.errorExists"),
            mode,
            LIST_STATUS.error
          );
          return;
  ...
  const fetchDataTime = () => {
    ScheduleSettingService.getDetailById({ id: user.stationsId }).then(
      (result) => {
        if (
          result &&
          result.stationBookingConfig &&
          result.stationBookingConfig.length > 0
        ) {
          let data = [
            {
  ...
  const fetchListData = (filter) => {
    ...
    ListSchedulesService.getList(filter).then((result) => {
      if (result) setDataSchedules(result);
      else
  ...
  const onDeleteSchedule = (id, message) => {
    ListSchedulesService.deleteSchedule({
  ...
  const convertStrToDate = (str) => {
    ...
    return !reasonableCalendarSwitching(dateFormat) ||
      reasonableCalendarSwitching(dateFormat) === "Invalid date"
      ? ""
      : dateFormat;
  };
  ...
  const handleExportExcel = async () => {
    let number = Math.ceil(
      dataListSchedules.data.length / DefaultFilterExport.limit
    );
    ...
    const newResult = results.map((item, index) => ({
      ...
      CustomerScheduleStatus:
        SCHEDULE_STATUS_STATES_EXPORT[item.CustomerScheduleStatus] || "",
      licensePlateColor:
        LICENSE_PLATE_COLOR_STATES_EXPORT[item.licensePlateColor] || "",
      ...
      onExportExcel({
        ...
        informationColumn: [
          ...
          [
            ...
            "",
            "",
            `Danh sách lịch hẹn ngày ${moment().format("DD/MM/YYYY")}`,
          ],
          [""],
        ],
        timeWait: 0,
              ...
              >
                ...
                <div className="col-md-6 col-lg-3 col-xl-2 mb-3">
                  <DatePicker.RangePicker
                    ...
                    defaultValue={[
                      moment(dataFilter.startDate, "DD/MM/YYYY"),
                      moment(dataFilter.endDate, "DD/MM/YYYY"),
                    ]}
                  />
                </div>
                ...
                <div className="col-md-6 col-lg-3 col-xl-2 mb-3">
                  <Select
                    ...
                    placeholder={translation(
                      "listSchedules.filterByScheduleType"
                    )}
                    style={{
                      minWidth: 160,
                    }}
                ...
                <div className="col-md-6 col-lg-3 col-xl-2 mb-3">
                  <Select
                    className="w-100"
                    placeholder={translation(
                      "listSchedules.filterByVehicleType"
                    )}
                    style={{
                      minWidth: 160,
                    }}
    ...
    <>
      ...
      {isVisible && (
        ...
        >
          ...
          />
          {error && (
            <div style={{ color: "#ff4d4f" }}>{translation("isReq")}</div>
          )}
          <div className="mt-2">
